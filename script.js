...
function initApp() {
    // MEZCLA ÚNICA: Solo mezclamos al iniciar la aplicación.
    // Esto evita que se "reinicie" el catálogo al cambiar de vistas.
    shuffleArray(moviesListInternal);
    shuffleArray(seriesListInternal);

    renderHomeView();
    
    // Para "Recién Agregadas" usamos el orden original de carga (globalContent) invertido
    let orderedContent = [];
...
function setupHero() {
    ...
    // Prioridad a la configuración ID.js
...
function renderHomeView() {
    // Usamos las listas ya mezcladas en initApp
...
function renderFilteredMovies() {
    ...
    // Usamos la lista original ya mezclada
...
function openModal(id, typeHint) {
    ...
    // Llama a la función de recomendados INTELIGENTE
...
function setPlayerVideo(url, overlayText = null) {
    ...
    // ATRIBUTO CRÍTICO: Bloquea popups y navegación
...
// --- RECOMENDADOS INTELIGENTES (POR GÉNERO) ---
function renderRealRecommendations(currentId) {
    ...
    // 1. Obtener géneros del item actual
    const currentGenres = getItemGenres(currentModalItem).map(g => g.toLowerCase().trim());

    // 2. Filtrar items que NO sean el actual
    let candidates = allContent.filter(i => String(i.id) !== String(currentId));

    // 3. Separar en "Relacionados" (comparten género) y "Otros"
    let related = [];
    let others = [];

    candidates.forEach(item => {
        const itemGenres = getItemGenres(item).map(g => g.toLowerCase().trim());
    ...
    // 4. Mezclar para variar
    shuffleArray(related);
    shuffleArray(others);

    // 5. Priorizar relacionados, rellenar si hace falta hasta completar 6
