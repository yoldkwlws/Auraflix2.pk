...
function iniciarTodo() {
    // 1. Inyectamos estilos visuales
    injectImageStyles();

    window.moviesList = window.moviesList || [];
    window.seriesList = window.seriesList || [];
    
    // 2. NUEVO: Pre-conectar a los servidores de imágenes para acelerar la descarga
    optimizeConnections();
...
// ==========================================
// OPTIMIZACIÓN DE RED (SPEED BOOST)
// ==========================================
function optimizeConnections() {
    // Detectamos los dominios de las primeras 20 imágenes y pre-conectamos
...
function injectImageStyles() {
    ...
    const css = `
        ...
        .smooth-image {
            opacity: 0;
            transition: opacity 0.4s ease-out; /* Transición un poco más rápida */
            will-change: opacity; /* Optimización de GPU */
        }
        /* Cuando cargan, se vuelven visibles */
        .smooth-image.loaded {
            opacity: 1;
        }
...
function renderHero() {
    ...
    // El Hero SIEMPRE es loading="eager" y high priority
...
function renderHomeView() {
    // Las primeras filas deben cargar YA (eager), las de abajo pueden esperar (lazy)
...
function renderList(containerId, list) {
    ...
    if (!container) return;
    // Renderizamos pasando el índice 'i' para controlar la prioridad de carga
...
function renderMultiRow(containerId, list) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        // Pasamos index 'j' para que las primeras de CADA fila horizontal carguen rápido
        rowDiv.innerHTML = chunk.map((item, j) => createItemHTML(item, j)).join('');
        container.appendChild(rowDiv);
    }
}

// OPTIMIZACIÓN CLAVE: createItemHTML ahora acepta 'index'
function createItemHTML(item, index = 100) {
    const type = item.seasons ? 'series' : 'movies';
    const safeId = String(item.id).replace(/'/g, "\\'");
    
    // ESTRATEGIA: Si es de los primeros 5 elementos (index < 5), cargamos EAGER (inmediato).
    // Si no, cargamos LAZY (cuando se vea).
    const loadingStrategy = index < 5 ? 'eager' : 'lazy';
    const fetchPriority = index < 5 ? 'high' : 'auto';
