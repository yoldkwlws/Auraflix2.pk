...
function initApp() {
    ...
    // CORRECCIÓN: Recién agregados ahora es ESTÁTICO (sin shuffle)
    const allContent = [...moviesListInternal, ...seriesListInternal];
    if (window.allContentSequence && window.allContentSequence.length > 0) {
        // Usamos el orden de carga inverso para ver lo más nuevo primero
...
function setupHero() {
    ...
    if (featuredList.length === 0) {
        ...
        featuredList = allFeatured.slice(0, 5); // Sin shuffle
    }
...
// CORRECCIÓN: renderHomeView ahora no mezcla los destacados de la Home
function renderHomeView() {
    // Usamos copias pero SIN llamar a shuffleArray
    const moviesStatic = [...moviesListInternal];
    const seriesStatic = [...seriesListInternal];

    // Se muestran en el orden en que aparecen en tus archivos .js
    renderMultiRow('homeMoviesRow', moviesStatic.slice(0, 30));
    renderMultiRow('homeSeriesRow', seriesStatic.slice(0, 30));
}
...
function switchView(viewName, pushToHistory = true) {
    ...
    if (viewName === 'home') {
        ...
        // Aquí puedes decidir si en la vista "Ver Todo" quieres shuffle o no
...
// El resto de funciones (renderMultiRow, openModal, etc.) se mantienen igual...
...
function shuffleArray(array) {
    // Esta función queda aquí por si la necesitas, pero ya no se usa en la Home
...
// --- MODAL --- (Copia el resto del código del modal que tenías antes)
...
function renderRealRecommendations(currentId) {
    ...
    // En recomendaciones sí dejamos el shuffle para que varíen
