...
document.addEventListener('DOMContentLoaded', () => {
    // Si id.js cargó correctamente, window.HERO_IDS debería estar disponible aquí o poco después
...
function initApp() {
    ...
    // Reverse para que el último archivo (peli21) salga primero
...
function setupHero() {
    ...
    if (window.HERO_IDS && Array.isArray(window.HERO_IDS) && window.HERO_IDS.length > 0) {
        window.HERO_IDS.forEach(config => {
            ...
            if (foundItem) {
                // Copiar item para no mutar el original
                const heroItem = { ...foundItem };
                // Sobreescribir imagen si existe en HERO_IDS
                if (customImg) {
    ...
    // Si NO se encontró nada en HERO_IDS o no estaba definido, usar fallback
    // Pero si HERO_IDS estaba definido, respetamos que solo salgan esos (aunque sean pocos)
    if (featuredList.length === 0) {
        // Solo si realmente no hay nada configurado, mostramos aleatorios/featured
...
function setupLatelyNew() {
    ...
    if (targetIds.length === 0) return; // Si no hay IDs, no mostramos nada (o podrías poner lógica fallback)
    ...
    // Mostramos exactamente lo que está en la lista
...
function openModal(id, typeHint) {
    ...
    if (g.length > 0) {
        ...
        // Si hay más de 3, agregamos puntos suspensivos
        if (g.length > 3) {
            genreDisplay += "...";
        }
    }
