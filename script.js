...
function checkLoginStatus() {
    if (!currentUser) {
        ...
        loadUserDataInUI();
        initApp(); // Intenta iniciar la app
    }
}

// =========================================================
// CORRECCIÓN DE CARGA: SISTEMA DE REINTENTO (RETRY)
// =========================================================
function initApp(retryCount = 0) {
    // Verificar si hay datos. Si las listas están vacías y llevamos pocos intentos, esperamos.
    const hasMovies = window.moviesList && window.moviesList.length > 0;
    const hasSeries = window.seriesList && window.seriesList.length > 0;

    if ((!hasMovies && !hasSeries) && retryCount < 10) {
        // console.log("Esperando datos...", retryCount); // Debug
        setTimeout(() => initApp(retryCount + 1), 300); // Reintentar en 300ms
        return;
    }

    // Si llegamos aquí, o hay datos o se acabaron los intentos.
    // Mezclamos solo si no se ha hecho antes
    if (!window.moviesList.shuffled && window.moviesList.length > 0) {
        shuffleArray(window.moviesList);
        window.moviesList.shuffled = true;
    }
    if (!window.seriesList.shuffled && window.seriesList.length > 0) {
    ...
    // Preparar lista unificada para "Recién agregadas"
    let orderedContent = [];
    ...
    // Renderizamos "Recién agregadas" (últimos 20)
...
// ==========================================
// FUNCIÓN MOSTRAR HERRAMIENTAS ADMIN
// ==========================================
...
// Función global para redirección desde el bloqueo
window.redirectToVipProfile = function() {
    closeModalInternal();
    switchView('profile');
    // Forzamos un scroll suave arriba para que vea el botón VIP
    window.scrollTo({top: 0, behavior: 'smooth'});
};

// =============================================================
// FUNCIÓN PLAYER VIDEO ESTÁNDAR (SIN BOTÓN EXPANDIR)
// =============================================================
...
async function checkLink(item, logContainer) {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            ...
            // Si falla el fetch con no-cors, el dominio no existe o está caído
...
function logToConsole(container, title, msg, type) {
    ...
    // Solo mostramos Título y Mensaje
