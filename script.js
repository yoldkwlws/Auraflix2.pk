...
function iniciarTodo() {
    ...
    // NUEVO: Activar sistema de rotación automática
    setupAutoRotation();
}

// NUEVO: Lógica para forzar horizontal en celulares al poner pantalla completa
function setupAutoRotation() {
    const handleRotation = async () => {
        // Detectar si hay algo en fullscreen
        ...
        if (isFullscreen) {
            // Intentar bloquear orientación a Landscape (Horizontal)
            try {
                ...
                // En PC o algunos navegadores esto falla, lo ignoramos silenciosamente
                console.log("Rotación automática no disponible en este dispositivo.");
            }
        } else {
            // Al salir, liberar la rotación
            try {
    ...
    // Escuchar eventos de fullscreen en varios navegadores
...
function initApp() {
    ...
    // 2. MEZCLAR SIEMPRE para las secciones generales
    shuffleArray(window.moviesList);
    shuffleArray(window.seriesList);

    renderHomeView();
    
    // 3. RENDERIZAR "RECIÉN AGREGADAS" CON ORDEN FIJO
    let newlyAdded = [];
    ...
    // Muestra los primeros 20 (los más nuevos según tu lista de JS)
...
function showVipBlocker() {
    ...
    // Prioriza 'banner' (horizontal) o 'backdrop' si existen en tu JS
...
function setPlayerVideo(url, overlayText = null) {
    ...
    if (isDirectFile) {
        ...
        // Atributo playsinline importante para móviles
...
function setupEventListeners() {
    ...
    // --- LÓGICA DE SOPORTE (Actualizada para el formulario) ---
    // El modal se abre y cierra igual, el envío se hace por formspree
...
window.addEventListener('popstate', (event) => {
    ...
    if (!event.state || !event.state.modal) {
        ...
        // CERRAR MODAL DE ACTUALIZACIÓN AL DAR ATRÁS
