...
function openModal(id, typeHint) {
    ...
    if (isSeries) {
        ...
        if (isVip) {
            ...
            setPlayerVideo(item.video, null, true); // Bloqueado
        }
...
function renderEpisodes(season, serieItem, autoPlayIndex = -1) {
    ...
    if (autoPlayIndex >= 0 && season.episodes[autoPlayIndex]) {
        ...
        if(isVip) {
            ...
            setPlayerVideo(ep.video, `T${season.season}: Cap ${ep.episode}`, true); // Bloqueado
        }
    }
...
function setPlayerVideo(url, overlayText = null, isBlocked = false) {
    ...
    // CARGAR VIDEO (SIEMPRE, INCLUSO SI ES VIP)
    if (isDirectFile) {
    ...
    // CAPA DE BLOQUEO (ENCIMA DEL VIDEO)
    if (isBlocked) {
        const blockerDiv = document.createElement('div');
        blockerDiv.className = 'vip-player-overlay'; // Clase del CSS con backdrop-filter
...
async function startDiagnosis(mode) {
    ...
    const BATCH_SIZE = 5; // Lotes más pequeños para evitar congelamientos
...
async function checkLink(item, logContainer) {
    ...
    try {
        ...
        // IMPORTANTE: fetch con no-cors devuelve status 0 (opaque)
        // No podemos saber si es 404 o 200, pero si falla la red o timeout, saltará al catch.
        await fetch(url, { 
            method: 'GET', 
            mode: 'no-cors', 
        ...
        // Si llega aquí, asumimos que "existe" (o al menos el servidor respondió algo)
        ...
        if (error.name === 'AbortError') {
            ...
            logToConsole(logContainer, item.title, "Caído / Error Red", "error");
        }
    }
}
