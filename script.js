...
function initApp() {
    // CORRECCIÓN PRINCIPAL: Mezclar las listas UNA SOLA VEZ al iniciar.
    // Esto asegura que el orden sea aleatorio pero se mantenga fijo mientras navegas.
    shuffleArray(moviesListInternal);
    shuffleArray(seriesListInternal);

    renderHomeView();
    
    // Para Recién Agregadas usamos globalContent para respetar el orden de llegada
    let orderedContent = [];
...
function setupHero() {
    ...
    // Prioridad Estricta: Si existe HERO_IDS en id.js, SOLO usamos esos.
    if (window.HERO_IDS && Array.isArray(window.HERO_IDS) && window.HERO_IDS.length > 0) {
        ...
        // Solo fallback si no hay config
...
function renderHomeView() {
    // Aquí YA NO mezclamos. Usamos las listas que mezclamos en initApp.
...
function renderFilteredMovies() {
    ...
    // Usamos moviesListInternal que YA está mezclada desde el inicio
    ...
    if (movieFilters.length > 0) {
        ...
        // ELIMINADO: shuffleArray(listToShow); -> Esto causaba que se movieran al volver
    }
...
function renderFilteredSeries() {
    ...
    // Usamos seriesListInternal que YA está mezclada
    ...
    if (seriesFilters.length > 0) {
        ...
        displayFilterText.style.display = 'none';
        // ELIMINADO: shuffleArray(listToShow);
    }
