// script.js - Final Corregido y Funcional con Reproductor
...
function createItemHTML(item) {
    ...
    return `<div class="item" onclick="openModal('${item.id}', '${type}')">
...
function openModal(id, type) {
    ...
    if (type === 'series') {
        ...
        actionBtn.onclick = () => {
            if(isPlayingTrailer) { setPlayerVideo(item.video); isPlayingTrailer=false; }
            else { setPlayerVideo(item.trailer,"Tráiler"); isPlayingTrailer=true; }
        };
    }
...
function renderRealRecommendations(currentId){
    ...
    container.innerHTML=allContent.slice(0,6).map(item=>`
        <div class="item" onclick="openModal('${item.id}','${item.seasons?'series':'movies'}')">
            <img src="${item.image}"><div class="item-title">${item.title}</div>
        </div>
    `).join('');
}
