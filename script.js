...
// --- LÓGICA DE HERO CORREGIDA (MODO ESTRICTO) ---
function setupHero() {
    const allContent = [...moviesListInternal, ...seriesListInternal];
    featuredList = [];
    let useCustomConfig = false;

    // Verificar si existe configuración en id.js
    ...
    // Solo usamos fallback si NO se intentó usar una configuración personalizada
    if (!useCustomConfig && featuredList.length === 0) {
        featuredList = allContent.filter(i => i.featured);
        if (featuredList.length === 0 && allContent.length > 0) {
            featuredList = allContent.slice(0, 5);
        }
    }
    // Si useCustomConfig es true pero featuredList está vacío (porque los IDs no coincidieron),
    // NO rellenamos con aleatorios. Se mostrará vacío para indicar que hay un error en los IDs.
...
function setPlayerVideo(url, overlayText = null) {
    ...
    // IFRAME LIMPIO para evitar bloqueos
    const iframe = document.createElement('iframe');
    iframe.src = url;
    iframe.setAttribute('allow', 'autoplay; fullscreen; encrypted-media; picture-in-picture');
    iframe.setAttribute('allowfullscreen', 'true');
    // REMOVIDO: referrerpolicy (causante de bloqueos en algunos servidores)
    ...
    if (overlayText) {
        ...
        overlay.style.pointerEvents = 'none'; // Evitar que el texto bloquee clicks en el video
