...
let selectedAvatarTemp = null; // Para la selección en el login
...
function renderAvatarSelection() {
    ...
    // profileImages viene de perfil.js
...
function loadUserDataInUI() {
    ...
    // Cargar imagen en el Nav Inferior
    const navImg = document.getElementById('navProfileImg');
    if(navImg) navImg.src = currentUser.avatar;

    // Cargar datos en la Vista de Perfil
...
document.getElementById('logoutBtn').addEventListener('click', () => {
    if(confirm("¿Seguro que quieres borrar tu perfil y reiniciar?")) {
...
function initApp() {
    ...
    setupHero(); 
    setupEventListeners();
    // switchView('home', false); // Ya se hace por defecto
}
...
function switchView(viewName, pushToHistory = true) {
    ...
    document.getElementById('view-profile').classList.add('hidden'); // Ocultar perfil
