...
function renderHomeView() {
    ...
    // CAMBIO SOLICITADO: Usar la nueva función para crear 3 filas de 10
    renderMultiRow('homeMoviesRow', moviesShuffled.slice(0, 30));
    renderMultiRow('homeSeriesRow', seriesShuffled.slice(0, 30));
}

// NUEVA FUNCIÓN: Divide una lista en hasta 3 filas de 10 elementos
function renderMultiRow(containerId, list) {
    const container = document.getElementById(containerId);
    if (!container) return;
    container.innerHTML = '';

    // Loop máximo 3 veces (para 3 filas verticales)
    for (let i = 0; i < 3; i++) {
        const start = i * 10;
        const end = start + 10;
        ...
        // Crear contenedor para esta fila horizontal
        const rowDiv = document.createElement('div');
        rowDiv.className = 'row horizontal-scroll'; // Reutilizamos estilo scroll horizontal
...
function openModal(id, type) {
    // CORRECCIÓN SOLICITADA: Cerrar el buscador si está abierto
    const searchOverlay = document.getElementById('searchOverlay');
    if(searchOverlay.style.display === 'block') {
        searchOverlay.style.display = 'none';
        // No quitamos modal-open aquí porque el modal lo necesitará
    }
