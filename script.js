...
function openModal(id, type) {
    ...
    // --- NUEVA LÓGICA DE AÑO/TIPO/GÉNEROS ---
    ...
    if (item.genres && Array.isArray(item.genres) && item.genres.length > 0) {
        const maxGenres = 4;
        let genreString = item.genres.slice(0, maxGenres).join('/');
        
        if (item.genres.length > maxGenres) {
            genreString += '...';
        }
        
        // El punto y los géneros en GRIS
        genresHtml = `<span class="meta-sub-gray">• ${genreString}</span>`;
    }

    // Año y Tipo en MORADO
    metaContainer.innerHTML = `
        <span class="meta-main-purple">${item.year} ${typeLabel}</span>
        ${genresHtml}
    `;
    // ----------------------------------------
